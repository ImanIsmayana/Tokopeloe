        ...
        panel "Recent Orders" do
          table_for Order.order("created_at desc").limit(3) do
            column :email do |order|
                link_to order.email, [:admin, order]
            end
            column :created_at
          end
          strong { link_to "View all Order ", admin_orders_path}
        end
        panel "Recent Product" do
          table_for Product.order("created_at desc").limit(3) do
          ...
          strong { link_to "View all Product ", admin_products_path}
        end
      end
    end # columns
